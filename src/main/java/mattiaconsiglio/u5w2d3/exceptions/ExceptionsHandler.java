...
import mattiaconsiglio.u5w2d3.payloads.ErrorDTO;
...
public class ExceptionsHandler {
    ...
    public ErrorDTO handleBadRequestException(BadRequestException e) {
        if (e.getErrorsList() != null) {
            return new ErrorDTO(e.getErrorsList().stream().map(objectError -> objectError.getDefaultMessage()).collect(Collectors.joining(". ")), LocalDateTime.now());
        }
        return new ErrorDTO(e.getMessage(), LocalDateTime.now());
    }
